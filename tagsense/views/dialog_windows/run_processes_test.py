...
class RunProcesses(QDialog):
    ...
    def run_selected_processes(self) -> None:
        ...
        # Check if any selected process is AppendText
        selected_processes = [self.process_list[i] for i in active_indices]
        is_append_text = any(
            process.__name__ == "AppendText"  # Check by class name
            for process in selected_processes
        )

    # Get text input if AppendText is selected
        custom_text = ""
        if is_append_text:
        ...
        if not ok or not text.strip():
            return  # Abort if user cancels or enters empty text
        ...
        def process_next(index: int) -> None:
            ...
            if checkbox.isChecked():
                ...
                try:
                    # Use custom text for AppendText processes
                    final_param = param_text
                    if process.__name__ == "AppendText":
                        final_param = custom_text

                    result = process.execute(db_path, final_param, output_callback)
                    # text, ok = QInputDialog.getText()
                    # result = process.execute(db_path, text, output_callback)
